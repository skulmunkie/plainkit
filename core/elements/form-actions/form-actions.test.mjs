// pk-form-actions is template and CSS only (no behaviour file), so this guards its API declaration: the meta file is valid, and every example
...
test('the meta file passes the API validator against its template and stylesheet', () => {
    ...
    assert.equal(meta.tag, 'pk-form-actions');
});
