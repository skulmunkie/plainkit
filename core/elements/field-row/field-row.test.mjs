// pk-field-row is template and CSS only (no behaviour file), so this guards its API declaration: the meta file is valid, and every example
...
const read = ext => fs.readFileSync(new URL(`./field-row.${ext}`, import.meta.url), 'utf8');
...
test('the meta file passes the API validator against its template and stylesheet', () => {
    ...
    assert.equal(meta.tag, 'pk-field-row');
});
