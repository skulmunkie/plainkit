...
export default Base => class extends Base {
    connected() {
        if (!this.$w) { this.$w = true; this.part('close')?.addEventListener('click', () => this.dismiss()); this.shadowRoot.addEventListener('click', e => { if (e.target.closest?.('[part="close"]')) this.dismiss(); }); }
    }
