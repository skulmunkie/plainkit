...
**What the sweep can and cannot see.** It measures each document's light DOM: overflow of the page, controls and text in the page's own markup and in slotted content, nested scrollers, and the level-1 heading count, which does look inside shadow trees. It does not walk shadow roots for text size or touch targets, so what an element draws inside itself (a `pk-button`'s inner button, a field's help text) is judged only through its host box; the SDK quality checks (`js/quality.js`, the `quality` stage) read accessible names from shadow text, and the `pages` stage runs them on the live page. Nested iframes (the gallery's sample previews) are measured by their own `sample` items, not from inside the gallery views.
