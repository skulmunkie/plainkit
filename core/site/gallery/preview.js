...
const { PATTERNS, LAYOUTS, CONTROLS, TEMPLATES } = data;

const entry = (() => {
    if (kind === 'patterns') { const p = PATTERNS.find(x => x.id === id); return p && { title: p.title, html: p.html }; }
    if (kind === 'layouts') {
        const l = id === 'shell' ? { title: 'App shell', html: CONTROLS.find(c => c.id === 'app-shell')?.samples[0].html } : LAYOUTS.find(x => x.id === id);
