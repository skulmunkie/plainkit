...
- **12 wrapper-only parameters do not exist** (for example `PkDialog.CloseButtonLabel`, `PkDrawer.IsLoading`, `PkTooltip.OnClick`), and `PkDialog.MaxWidthPx`, `PkDialog.Theme`, `PkTooltip.Kind`, `PkChart.Data` and `PkImageGallery.Images` are not generated. `references/known-gaps.md` has the full list; do not use a parameter that is not in `references/components-*.md`.
