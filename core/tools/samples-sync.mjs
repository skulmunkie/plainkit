...
import { classOwners, usedComponents, elementFolders } from './usage.mjs';

const root = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '..');
const owners = classOwners(root); const elements = elementFolders(root);
let changed = 0;
for (const [, dir] of SAMPLE_GROUPS) {
    for (const e of fs.readdirSync(path.join(root, dir), { withFileTypes: true }).filter(d => d.isDirectory())) {
        ...
        const used = usedComponents(markup, owners, elements);
