...
| --- | --- | --- |
| Drag-and-drop / sortable with a drop indicator | `pk-sortable`, `pk-sortable-item` exist (#170) | **Wired up for the top-level page order and palette inserts (#174).** The top level is a real `pk-sortable` of `pk-sortable-item` rows: a pointer or touch drag on a row's handle reorders the page (`pk-reorder` drives `M.moveNode`), and a palette button is a second drag source using the external-drop API, landing between top-level rows or, hit-tested onto a container, inside its default slot (slot-aware). Reordering *inside* a container, or moving a node into/out of one, stays keyboard/toolbar (Alt+arrows, Out/In): nesting `pk-sortable` inside a container's own slot would put its `pk-sortable-item` between the container and the many elements whose shadow CSS keys off `::slotted()` directly (`pk-stack`'s dividers, `pk-card`'s `[slot="media"]`, and 30-odd others), breaking their styling, so it was left out rather than shipped unverified. |
...
| 5c | Not built yet: reordering *inside* a container (nesting `pk-sortable` would break `::slotted()` styling across ~30 elements, see the gap table above), a compact context menu if Duplicate/Wrap need one later, a phone-reachable Save, the structure tree wired to drag, the iframe device preview, the blocks UI (`blocks`, `onblock`), `draftKey`, the Blazor wrapper `PkLayoutBuilder` and its Razor exporter. | later |
