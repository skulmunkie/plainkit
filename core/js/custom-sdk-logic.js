...
// ---- the manifest (the build writes dist/manifest.json with this too)

export const DEFAULT_PROVENANCE = 'Generated by sdk/tools/build.mjs from the sources; reproducible.';

/** dist/manifest.json as text (LF; the build and the export convert to the files' line endings): sorted files, no timestamps. files: [{ path, bytes, integrity }]. */
export function manifestText({ version, files, provenance = DEFAULT_PROVENANCE }) {
    const sorted = [...files].sort((a, b) => (a.path < b.path ? -1 : a.path > b.path ? 1 : 0));
    return JSON.stringify({ name: 'plainkit', version, dependencies: [], runtimeRequests: 'none (same-origin only)', licence: 'MIT', provenance, files: sorted }, null, 2) + '\n';
}
...
export function readmeText({ version, include, breakpoints, shipped, overrideCount, themeOnly }) {
    ...
    if (themeOnly) {
        ...
        lines.push('## Use it', '',
            'The `dist/` folder has the release layout: use it wherever the release `dist/` is used (copy it next to your page, or serve it, and point the `<link>` and `<script>` at it).',
            ...
            '- `dist/manifest.json` was recomputed: each file has its size and SRI hash (`integrity="sha384-..."`).', '');
    }
...
export async function customizeDist(files, { include, breakpoints, theme }) {
    ...
    if (changed || themeCss) {
        for (const [path, bytes] of files) {
            if (path === 'manifest.json' || !isText(path)) continue;
    ...
    const entries = [];
    for (const [path, bytes] of [...out].filter(([p]) => p !== 'manifest.json')) entries.push({ path, bytes: bytes.length, integrity: await integrityOf(bytes) });
    const custom = changed || Boolean(themeCss);
    const provenance = custom ? `Customised export of Plainkit ${manifest.version} (see ${SETTINGS_NAME}); every file listed here is exactly what this bundle holds.` : manifest.provenance;
    out.set('manifest.json', enc.encode(eol(manifestText({ version: manifest.version, files: entries, provenance }), dec.decode(manifestBytes))));
    return { files: out, summary: { version: manifest.version, shipped, breakpoints: to, changedBreakpoints: changed, themeBaked: Boolean(themeCss), counts, touched: touched.sort(), fileCount: entries.length + 1 } };
}
