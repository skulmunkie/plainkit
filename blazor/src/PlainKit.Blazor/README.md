...
What the table element does not do yet (a core request, tracked in the issues): it has no current-row highlight, so the first cell of the current row is marked with bold and `aria-current` instead. Rows are keyboard stops, a third click on a sortable header clears the sort, and a `HidePhone` column is hidden in the `cards` layout too.
