...
- **Long-lived caching.** Call `app.MapStaticAssets()` (the default in the ASP.NET Core 9+ project templates; `blazor/samples/PlainKit.Playground` does, the only sample with a server) and `<PkStyles />` (or a `<Pk*>` element, or the dev tools) automatically links a **fingerprinted** URL instead: ASP.NET Core's static asset pipeline already fingerprints every file the package serves (it is on by default, `StaticWebAssetFingerprintingEnabled`), and `Assets[...]` (`Microsoft.AspNetCore.Components.Web`, a `ComponentBase` member) resolves it. A fingerprinted URL is served with `Cache-Control: public, max-age=31536000, immutable`, so a warm visit fetches it once, not on every load. `PkRuntime.EnsureInitializedAsync(Assets)` and `PkRuntime.BridgeAsync(Assets)` do the same for the JavaScript bridge (`plainkit.blazor.js`); every generated `pk-*` component and the dev tools already pass `Assets` when they call in. Writing your own `<link>`/`<script>` for a toolkit file the same way (`@Assets["_content/PlainKit.Blazor/plainkit/js/plainkit.js"]`) gets the same immutable caching; a dynamic `import()` needs the leading `./` that `Assets[...]` does not add back.
- **Cache busting (without `MapStaticAssets`, or from non-component code).** `PkAssets.Versioned("js/plainkit.js")` (any file of the toolkit, relative to `PkAssets.Root` or a full path) appends `?v=` and the first 12 characters of the file's SHA-384, so a browser fetches the file again exactly when its content changed — still revalidated on every visit (`Cache-Control: no-cache`), unlike the fingerprinted URL above. The hash comes from the package's manifest, embedded in the assembly and read once: no file access per request. `PkAssets.Integrity(path)` gives the full `sha384-...` value for an `integrity` attribute. `PkStyles` and `PkRuntime` fall back to this when there is no `Assets` to fingerprint with (`Versioned="false"` turns cache busting off entirely).
- **Preloading.** `<PkStyles Preload="true" />` also emits a `<link rel="modulepreload">` for each of `PkAssets.PreloadPaths` (the small entry, the loader, its logger, the element registry and the base every element shares), so the browser fetches them in parallel with the entry module instead of one at a time as each is discovered. Off by default; worth turning on for a page that uses `pk-*` elements.
