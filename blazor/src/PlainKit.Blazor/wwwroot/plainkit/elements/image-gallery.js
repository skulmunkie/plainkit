...
export default define(class extends behaviour(PkElement) {
    ...
    static css = ":host{display: block;inline-size: 100%;min-inline-size: 0}[part=\"grid\"]{list-style: none;margin: 0;padding: 0;display: grid;grid-template-columns: repeat(var(--_cols,auto-fill),minmax(var(--_min,6.25rem),1fr));gap: var(--space-3)}[part=\"tile\"]{position: relative;display: grid;grid-template-rows: auto 1fr;min-inline-size: 0;overflow: hidden;border: 1px solid var(--color-border);border-radius: var(--pk-image-gallery-radius,var(--radius-lg));background: var(--color-surface-alt)}[part=\"tile\"][data-primary]{border: 2px solid var(--color-accent)}[part=\"media\"]{display: block}[part=\"badge\"],[part=\"status\"]{position: absolute;inset-block-start: var(--space-1)}[part=\"badge\"]{inset-inline-start: var(--space-1)}[part=\"status\"]{inset-inline-end: var(--space-1)}[part=\"actions\"]{display: flex;flex-wrap: wrap;justify-content: center;gap: var(--space-1);padding: var(--space-1);background: var(--color-panel);border-top: 1px solid var(--color-border)}[part=\"action\"]{flex: 1 1 auto}[part=\"add-tile\"]{position: relative;display: grid;align-self: start;aspect-ratio: 1;min-inline-size: 0}[part=\"add\"]{position: relative;display: flex;flex-direction: column;align-items: center;justify-content: center;gap: var(--space-1);padding: var(--space-2);border: 1px dashed var(--color-border);border-radius: var(--pk-image-gallery-radius,var(--radius-lg));background: var(--color-surface-alt);color: var(--color-muted);text-align: center;cursor: pointer;touch-action: manipulation}[part=\"add\"]:hover{border-color: var(--color-accent);color: var(--color-text)}[part=\"add\"]:focus-within{outline: var(--focus-ring);outline-offset: 2px}[part=\"add-icon\"]{font-size: var(--text-lg);line-height: 1}[part=\"add-text\"]{font-size: var(--text-meta);line-height: 1.2}[part=\"file\"]{position: absolute;inset: 0;inline-size: 100%;block-size: 100%;opacity: 0;cursor: pointer}@media (max-width: 640px){[part=\"action\"]{--pk-button-padding: calc((var(--touch-target) - 1.25 * var(--text-meta)) / 2) var(--space-2)}[part=\"add\"]{min-block-size: var(--touch-target)}}";
});
