...
public sealed class DialogSizeTests : TestContext
{
    ...
    {
        var cut = RenderComponent<PkDialog>(p => p.Add(x => x.Size, size));
    ...
    {
        var cut = RenderComponent<PkDialog>();
    ...
    {
        var cut = RenderComponent<PkDialog>(p => p.Add(x => x.MaxWidthPx, 720));
