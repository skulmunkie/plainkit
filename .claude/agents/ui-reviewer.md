...
4. For every scenario in `manifest.scenarios`, read its file in `core/tests/review/scenarios/` (the steps say how the state was reached, `expect(t)` says what was measured), open each of its screenshots, and check that the picture agrees with the numbers and that the issue's expectation is actually one of the assertions. A finding with rule `scenario-expectation` is an error (the state is wrong), `known issue #N` is a defect already filed. A state the issue describes that no scenario reaches goes under "what I could not see".
5. Write the feedback below.
...
4. **What I could not see**: name every combination (element, example, desktop or phone, light or dark) that has no screenshot or that you did not open, everything in `notSeen`, and what a still image cannot show (animation, real data, other widths between 375 and 1280; hover, focus, scrolling and right-to-left only when a scenario covers them, and say which scenario). Never write that something is fine if you did not see it.
5. **Suggested next steps** for the owner: what to look at first, and which expectation should become a measuring browser case (`core/tests/browser/`) or a scenario assertion (`core/tests/review/scenarios/`) if it is not one yet, and which defect you would file as an issue (screenshot name and steps).
