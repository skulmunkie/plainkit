...
const SDK_GAPS = [
    ...
    'The Guides (the Guides page of the site, from Markdown in core/site/guides/content) are a first set of seven: getting started with the SDK and with Blazor, choosing what to build with, theming and tokens, responsive design and breakpoints, logging, migrating from the compat classes. The page has a search box (title and full-text, from a build-time word index), but no versions and no HTML/Blazor tabs yet, so the references in this skill remain the full documentation.',
...
function upgradingMd(src, findVersion) {
    return ['# Upgrading', '', stamp(src, 'CHANGELOG.md and core/tools/api-surface.mjs'), '',
        ...
        '## 3b. Let the app tell you about removed CSS classes', '',
        `Removed classes (retired class-based components, removed \`u-*\` utilities) are silent: nothing fails, the styling just disappears, and a grep for names you did not know to look for misses them. \`js/compat-warn.js\` is an opt-in module (zero cost unless imported): \`import { checkCompatClasses } from './plainkit/js/compat-warn.js'; checkCompatClasses();\` in a dev build logs each retired or removed class it finds in the running app, once, with a replacement (scope \`compat\`, warn level). Load the app's main pages with it on and add every warning to the checklist. Do not ship the call to production.`, '',
...
// the use-case table and the anti-patterns; this flattens its links (a skill has no Guides page), drops the Blazor section from the SDK skill, and
// appends the catalogue of templates, layouts and patterns from their own meta files so the names cannot go stale.
export const CHOOSING_GUIDE = 'core/site/guides/content/choosing-what-to-build-with.md';
function choosingMd(src, skill) {
    let body = lf(fs.readFileSync(path.join(root, CHOOSING_GUIDE), 'utf8')).replace(/^---\n[\s\S]*?\n---\n/, '').trim();
    body = body.replace(/\[([^\]]+)\]\([^)\s]+\)/g, '$1');
    if (skill === 'plainkit-sdk') body = body.replace(/^## In Blazor\n[\s\S]*?(?=^## )/m, '');
    const cat = (title, list) => [`## Catalogue: ${title}`, '', ...list.map(t => `- ${code(t.id)}: ${t.summary}`), ''];
    return ['# Choose before you build', '', stamp(src, `${CHOOSING_GUIDE} and the template, layout and pattern meta files`), '',
        'Before you write markup for a page: (1) name the page type or job, (2) look it up in the use-case table below, (3) open the template, layout, pattern or element it names, (4) copy it and change content, slots, props and tokens only. Prefer a template over a layout, a layout over a pattern, a pattern over your own composition, an element over a `<div>`. If nothing fits, say so and record the gap; do not invent a tag or a component.', '',
        body, '',
...
function blazorFiles(src) {
    ...
    if (fg) {
        files.set('references/field-group.md', ['# PkFieldGroup: a plain field bound to a model property, from a list of specs', '', stamp(src, 'Components/PkFieldGroup.razor and PkFieldGroupTypes.cs'), '',
            'A hand-written component with no element of its own. Renders a `PkField` plus the right control (`PkInput`, `PkSelect`, `PkTextarea` or `PkCheckbox`) for each `PkFieldSpec<TItem>`, reading and writing `Model` through the spec\'s `Get`/`Set` (`Func<TItem, string?>`/`Action<TItem, string?>`: every control\'s own value-shaped parameter is a string, so a field spec deals in the same currency; `PkInput.Min`/`Max`/`Step`/`MaxLength` are strings for the same reason). A field can gate its own rendering with `When` (`Func<TItem, bool>`), re-evaluated on every render; a field whose value is computed rather than a plain mirror still stays hand-written next to it, the same `PkField` plus control you would already write. Composes with `PkForm`\'s own validation with no extra wiring: every control it renders is the same real element a hand-written field would use, and a field hidden by `When` renders no markup at all, so a hidden `Required` field never blocks submission.', '',
            '```razor', '<PkForm>', '    <PkFieldGroup TItem="Order" Fields="_fields" Model="_order" ModelChanged="Save" />', '</PkForm>', '', '@code {', '    private Order _order = new();', '    private IReadOnlyList<PkFieldSpec<Order>> _fields =', '    [', '        new() { Key = "name", Label = "Name", Required = true, Get = order => order.Name, Set = (order, v) => order.Name = v ?? "" },', '        new() { Key = "qty", Label = "Quantity", Kind = PkFieldKind.Number, Min = "1", Get = order => order.Qty.ToString(), Set = (order, v) => order.Qty = int.Parse(v ?? "0") },', '        new() { Key = "status", Label = "Status", Kind = PkFieldKind.Select, Options = [new("open", "Open"), new("closed", "Closed")], Get = order => order.Status, Set = (order, v) => order.Status = v ?? "" },', '        new() { Key = "note", Label = "Closing note", Required = true, When = order => order.Status == "closed", Get = order => order.Notes, Set = (order, v) => order.Notes = v ?? "" },', '    ];', '}', '```', '',
            '## `PkFieldGroup`', '', table(['Parameter', 'Type', 'Description'], fg.params.map(p => [code(p.name), code(p.type), p.doc])), '',
            '## `PkFieldSpec<TItem>`', '', 'One field: `Key`, `Label`, `Get` (`Func<TItem, string?>`), `Set` (`Action<TItem, string?>`), `Kind` (`PkFieldKind`, default `Text`), `Hint`, `Required`, `Min`, `Max`, `Step`, `MaxLength`, `Pattern`, `Placeholder` (`PkInput`/`PkTextarea`), `Rows` (`int?`, `Textarea` only), `Help` (text in a help `PkTooltip` beside the label), `Options` (`IReadOnlyList<PkFieldOption>`, `Select` only), `Key` is also emitted as each control\'s `name`. For a `bool` property use `PkFieldSpec<TItem>.Bool(key, label, item => item.Active, (item, v) => item.Active = v)` (a `Checkbox` spec wrapping the string `Get`/`Set`; add other members with `with { ... }`). `When` (`Func<TItem, bool>`, unset means always rendered; re-run for every field on every render, since one field\'s value can gate another\'s visibility).', '',
            '`Disabled` and `ReadOnly` (`Func<TItem, bool>`, per render: a code editable while the record is new and locked after; `ReadOnly` keeps the value selectable, on `PkSelect`/`PkCheckbox` it disables), `Span` (`bool`: the field takes every column of the `pk-form-section` grid, the `form-span` class), `OptionsSource` (`Func<IReadOnlyList<PkFieldOption>>`, read at every render, wins over `Options`; for lists that load later), `HideLabel` (`bool`: the label is not shown but is the control\'s accessible name) `HelpWhen` (`Func<TItem, string?>`: help per record, used instead of `Help` when non-empty) and `LabelAction` (`RenderFragment<TItem>`: extra content in the label-action slot beside the label, after the Help tooltip and re-evaluated each render; for adopt-from-source buttons and flag badges, never replaces the label text). The help button\'s accessible name is "Help for <Label>"; for a `Checkbox` the tooltip sits beside the field label, like any other field. Not in the spec yet: an async `Set`, an input `Format`.', '',
            '`ModelChanged` is raised after `Set` has updated the model. It is an `EventCallback`, so a handler on the parent re-renders the parent and the group with it; give the group a handler (even an empty method) when other UI on the page depends on a field.', '',
...
@code {
    ...
    if (flr) {
        files.set('references/field-list-row.md', ['# PkFieldListRow: an optional term/value row for PkFieldList', '', stamp(src, 'Components/PkFieldListRow.razor'), '',
            'A hand-written component with no element of its own. `PkFieldList`\'s slot just takes raw `dt`/`dd` pairs; `PkFieldListRow` composes the "should this row even show up" decision so an optional value does not need an `@if` wrapper at every call site. Not named `PkField`: that is already the generated wrapper for the `pk-field` form-layout element (label/help/error around a control), an unrelated concept. `Value` (plain text) and `ChildContent` (rich content, e.g. a badge or link) are mutually exclusive; when both are set, `ChildContent` wins.', '',
            'Visibility: `When`, when set (non-null), wins outright over `SkipEmpty` — `true` renders the row even with nothing to show, `false` hides it even with a value present. Left `null` (the default), visibility falls back to `SkipEmpty` (default `true`): the row is omitted when there is no `Value` and no `ChildContent`.', '',
            '```razor', '<PkFieldList Title="Order">', '    <PkFieldListRow Label="Number" Value="@order.Number" />', '    <PkFieldListRow Label="Note" Value="@order.Note" />', '    <PkFieldListRow Label="Status"><PkBadge>@order.Status</PkBadge></PkFieldListRow>', '</PkFieldList>', '```', '',
            '## `PkFieldListRow`', '', table(['Parameter', 'Type', 'Description'], flr.params.map(p => [code(p.name), code(p.type), p.doc])), ''].join('\n'));
    }
    ...
    if (rt) {
        files.set('references/raw-table.md', ['# PkRawTable: HeadContent/ChildContent/FootContent composed into pk-table\'s raw slot', '', stamp(src, 'Components/PkRawTable.razor'), '',
            ...
            '`class` and other attributes go to the `pk-table` frame, `TableClass` to the inner table. Set `IsEmpty` with `EmptyText` or `EmptyContent` to show that instead of a header over no rows.', '', '## `PkRawTable`', '', table(['Parameter', 'Type', 'Description'], rt.params.map(p => [code(p.name), code(p.type), p.doc])), ''].join('\n'));
    }
...
@code {
    ...
    files.set('references/record-editor.md', ['# PkRecordEditor: the state of a create-or-edit record page', '', stamp(src, 'PkRecordEditor.cs'), '',
        ...
        '## Which errors the user sees', '', 'An exception thrown by `save` or `delete` that implements the marker interface `IPkUserFacingException` (no members) shows its `Message` as it is: a domain rule written for the person ("Only one location can be primary."). Put the marker on the exception base type the app already has (`class AppException(string message) : Exception(message), IPkUserFacingException;`), so every rule that derives from it is shown. When the domain layer must not reference PlainKit.Blazor, set the optional `IsUserFacing` (`Func<Exception, bool>`, an init property: `new PkRecordEditor<...>(...) { IsUserFacing = ex => ex is AppException }`); it is consulted in addition to the marker, so either one shows the message, and a predicate that throws is logged and counts as false. Any other exception is logged (`LogError` with the verb and noun) and the user sees "Unexpected error while saving. See log for details." (or "deleting"): an exception message is not written for users and can leak internals.', ''].join('\n'));
    ...
    files.set('references/setup-and-options.md', ['# Setup, options and services', '', stamp(src, 'PkOptions.cs, PkLogging.cs, PkTimeZone.cs, PkRuntime.cs, PkAssets.cs and the extension methods'), '',
        ...
        members('PkOptions', src.cs.options), members('PkLoggingOptions', src.cs.logging), members('IPkLog', src.cs.ipklog), members('PkRuntime', src.cs.runtime), members('PkAssets', src.cs.assets),
        '## Display timezone', '', 'Inject `IPkTimeZoneResolver` to render every timestamp in one fixed, configured zone regardless of the host\'s own OS timezone. Set `PkOptions.TimeZone.IanaId` (tried first) and `WindowsId` (tried when the IANA id does not resolve, e.g. on a trimmed or globalization-invariant host); set `BaseUtcOffset`, `StandardName`, `DaylightName` and `AdjustmentRules` to describe a synthetic fallback zone used when neither id resolves, so the feature degrades instead of throwing. `Resolution.Source` (`Iana`, `Windows`, `Fallback`) says which path was taken, and a `Windows` or `Fallback` resolution is also logged as a warning. `ToConfiguredZone(DateTime)` converts, treating `DateTimeKind.Unspecified` as UTC (the common case for a timestamp read back from a database column with no kind attached), not the host\'s local kind.', '',
        members('PkTimeZoneOptions', src.cs.timeZoneOptions), members('IPkTimeZoneResolver', src.cs.timeZoneResolver)].join('\n'));
