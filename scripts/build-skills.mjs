...
function blazorFiles(src) {
    ...
    if (fg) {
        files.set('references/field-group.md', ['# PkFieldGroup: a plain field bound to a model property, from a list of specs', '', stamp(src, 'Components/PkFieldGroup.razor and PkFieldGroupTypes.cs'), '',
            ...
            '## `PkFieldSpec<TItem>`', '', 'One field: `Key`, `Label`, `Get` (`Func<TItem, string?>`), `Set` (`Action<TItem, string?>`), `Kind` (`PkFieldKind`, default `Text`), `Hint`, `Required`, `Min`, `Max`, `Step`, `MaxLength`, `Pattern`, `Options` (`IReadOnlyList<PkFieldOption>`, `Select` only), `When` (`Func<TItem, bool>`, unset means always rendered; re-run for every field on every render, since one field\'s value can gate another\'s visibility).', '',
