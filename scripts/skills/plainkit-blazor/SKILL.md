...
- No inline styles or scripts: the toolkit is built for a strict CSP. The components respond on their own at the SDK's named breakpoints ({{breakpoints}} px, desktop-first: a rule applies at that width and below). In your own CSS write the literal query with the same width (`@media (max-width: 640px)`); the widths are also `--pk-bp-phone`, `--pk-bp-tablet` and `--pk-bp-wide` on `:root`. Other widths need a rebuild of the SDK.
